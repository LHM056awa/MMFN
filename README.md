...
网络舆情数据/网络舆情监测任务具有**模态异质、语义稀疏、真伪交织**的特点，现有基于多模态特征融合的舆情监测方法，采用**全局特征拼接与晚期融合**机制，但是难以应对实际数据的**细粒度语义对齐与跨模态干扰**特性。本项目创新性地提出基于**多粒度多模态特征融合**的舆情监测方法，结合**跨模态注意力与细粒度交互**的优势，提取**局部-全局协同的跨模态语义特征**，并基于此预测新闻与**多源证据**的适配性，解决了**模态间信息不对齐与细粒度证据利用不足**的问题。项目所提算法有效应用于（微博平台虚假新闻检测任务），取得了一定改进，具有良好的应用前景。
